<template>
  <div class="panel-root">
    <div class="header-actions">
      <el-button type="primary" class="mac-button-blue" @click="openAdd">
        <el-icon><Plus /></el-icon>
        新增年检
      </el-button>
      <div class="search-group">
        <el-input
          v-model="query.certNo"
          placeholder="证书编号"
          clearable
          class="search-input"
          @keyup.enter="loadList"
        />
        <el-select v-model="query.vehicleId" placeholder="车辆" clearable filterable class="search-select">
          <el-option v-for="item in vehicleOptions" :key="`c-v-${item.id}`" :label="item.label" :value="item.id" />
        </el-select>
        <el-select v-model="query.status" placeholder="状态" clearable class="search-select">
          <el-option v-for="item in statusOptions" :key="`c-s-${item.value}`" :label="item.label" :value="item.value" />
        </el-select>
        <el-button class="mac-button-gray" @click="loadList">搜索</el-button>
      </div>
    </div>

    <el-table :data="list" v-loading="loading" height="100%">
      <el-table-column type="index" label="序号" width="60" align="center" />
      <el-table-column prop="certNo" label="证书编号" min-width="140" />
      <el-table-column label="年检车辆" min-width="160">
        <template #default="{ row }">{{ getVehicleText(row.vehicleId) }}</template>
      </el-table-column>
      <el-table-column prop="inspectYear" label="年度" min-width="90" align="center" />
      <el-table-column prop="inspectTime" label="年检时间" min-width="160" align="center" />
      <el-table-column prop="validUntil" label="有效期" min-width="120" align="center" />
      <el-table-column label="状态" min-width="90" align="center">
        <template #default="{ row }">
          <el-tag :type="isRiskStatus(row.status) ? 'danger' : 'success'" size="small" effect="light">
            {{ getStatusText(row.status) }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="fee" label="费用(元)" min-width="95" align="center" />
      <el-table-column prop="location" label="地点" min-width="140" show-overflow-tooltip />
      <el-table-column label="操作" width="130" fixed="right" align="right">
        <template #default="{ row }">
          <el-button link type="primary" @click="openEdit(row)">编辑</el-button>
          <el-button link type="danger" @click="handleDelete(row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>

    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="760px" class="mac-dialog" destroy-on-close>
      <el-form ref="formRef" :model="form" :rules="rules" label-position="top">
        <el-row :gutter="14">
          <el-col :span="12">
            <el-form-item label="证书编号" prop="certNo"><el-input v-model="form.certNo" /></el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="年检车辆" prop="vehicleId">
              <el-select v-model="form.vehicleId" filterable style="width: 100%">
                <el-option
                  v-for="item in vehicleOptions"
                  :key="`ic-v-${item.id}`"
                  :label="item.label"
                  :value="item.id"
                />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="年检年度" prop="inspectYear"><el-input v-model="form.inspectYear" /></el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="年检状态" prop="status">
              <el-select v-model="form.status" style="width: 100%">
                <el-option v-for="item in statusOptions" :key="`ic-s-${item.value}`" :label="item.label" :value="item.value" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="年检时间" prop="inspectTime">
              <el-date-picker
                v-model="form.inspectTime"
                type="datetime"
                value-format="YYYY-MM-DD HH:mm:ss"
                style="width: 100%"
              />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="有效期" prop="validUntil">
              <el-date-picker v-model="form.validUntil" type="date" value-format="YYYY-MM-DD" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="年检项目" prop="inspectItem"><el-input v-model="form.inspectItem" /></el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="年检费用(元)" prop="fee">
              <el-input-number v-model="form.fee" :min="0" :precision="2" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="年检地点" prop="location"><el-input v-model="form.location" /></el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="年检人员" prop="inspector"><el-input v-model="form.inspector" /></el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="送检人员"><el-input v-model="form.sender" /></el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="备注"><el-input v-model="form.remark" type="textarea" :rows="2" /></el-form-item>
          </el-col>
        </el-row>
      </el-form>
      <template #footer>
        <el-button class="mac-button-gray" @click="dialogVisible = false">取消</el-button>
        <el-button type="primary" class="mac-button-blue" :loading="submitting" @click="submit">保存</el-button>
      </template>
    </el-dialog>
  </div>
</template>

<script setup>
import { ref, reactive, computed, onMounted } from 'vue'
import { Plus } from '@element-plus/icons-vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import { selectVehicle } from '@/api/vehicle'
import {
  selectVehicleInspection,
  saveVehicleInspection,
  deleteVehicleInspection,
} from '@/api/vehicleInspection'
import {
  toDictValue,
  buildDictLabelMap,
  findDictLabel,
  loadDictOptionsByCode,
} from '@/utils/dict'

const toKey = toDictValue

const STATUS_FALLBACK = [
  { label: '通过', value: '10', sort: 10 },
  { label: '未通过', value: '20', sort: 20 },
]

const query = reactive({ certNo: '', vehicleId: null, status: '' })
const list = ref([])
const loading = ref(false)

const dialogVisible = ref(false)
const dialogTitle = ref('新增年检')
const submitting = ref(false)
const formRef = ref(null)
const form = reactive({
  id: null,
  certNo: '',
  validUntil: '',
  inspectYear: '',
  inspectTime: '',
  inspectItem: '',
  fee: 0,
  location: '',
  vehicleId: null,
  inspector: '',
  sender: '',
  status: '10',
  remark: '',
})

const rules = {
  certNo: [{ required: true, message: '证书编号不能为空', trigger: 'blur' }],
  validUntil: [{ required: true, message: '有效期不能为空', trigger: 'change' }],
  inspectYear: [{ required: true, message: '年检年度不能为空', trigger: 'blur' }],
  inspectTime: [{ required: true, message: '年检时间不能为空', trigger: 'change' }],
  inspectItem: [{ required: true, message: '年检项目不能为空', trigger: 'blur' }],
  fee: [{ required: true, message: '年检费用不能为空', trigger: 'change' }],
  location: [{ required: true, message: '年检地点不能为空', trigger: 'blur' }],
  vehicleId: [{ required: true, message: '请选择年检车辆', trigger: 'change' }],
  inspector: [{ required: true, message: '年检人员不能为空', trigger: 'blur' }],
  status: [{ required: true, message: '请选择年检状态', trigger: 'change' }],
}

const vehicleOptions = ref([])
const statusOptions = ref([])

const vehicleMap = computed(() => {
  const map = {}
  ;(vehicleOptions.value || []).forEach((item) => {
    map[String(item.id)] = item
  })
  return map
})
const statusMap = computed(() => buildDictLabelMap(statusOptions.value))

const getVehicleText = (vehicleId) => {
  const item = vehicleMap.value[String(vehicleId)]
  if (!item) return `车辆ID:${vehicleId || '--'}`
  return `${item.license}（${item.brand || '--'}）`
}

const getStatusText = (value) =>
  statusMap.value[toKey(value)] || findDictLabel(statusOptions.value, value, toKey(value) || '--')

const isRiskStatus = (value) => {
  const label = getStatusText(value)
  return ['未通过', '不通过', '失败', '驳回', '异常'].some((text) => label.includes(text))
}

const getDefaultStatus = () => statusOptions.value[0]?.value || '10'

const resetForm = () => {
  Object.assign(form, {
    id: null,
    certNo: '',
    validUntil: '',
    inspectYear: '',
    inspectTime: '',
    inspectItem: '',
    fee: 0,
    location: '',
    vehicleId: null,
    inspector: '',
    sender: '',
    status: getDefaultStatus(),
    remark: '',
  })
}

const normalizeEditForm = (row) => {
  Object.assign(form, {
    ...row,
    fee: Number(row?.fee || 0),
    status: toKey(row?.status),
  })

  if (!statusOptions.value.some((item) => item.value === toKey(form.status))) {
    form.status = getDefaultStatus()
  }
}

const loadVehicleOptions = async () => {
  const res = await selectVehicle({})
  vehicleOptions.value = (res?.data || []).map((item) => ({
    id: item.id,
    license: item.license || `车辆-${item.id}`,
    brand: item.brand || '',
    label: `${item.license || `车辆-${item.id}`}（ID:${item.id}）`,
  }))
}

const loadDictOptions = async () => {
  statusOptions.value = await loadDictOptionsByCode('110', STATUS_FALLBACK)

  if (!statusOptions.value.some((item) => item.value === toKey(form.status))) {
    form.status = getDefaultStatus()
  }
}

const loadList = async () => {
  loading.value = true
  try {
    const res = await selectVehicleInspection(query)
    list.value = res?.data || []
  } finally {
    loading.value = false
  }
}

const openAdd = () => {
  dialogTitle.value = '新增年检'
  resetForm()
  dialogVisible.value = true
  if (formRef.value) formRef.value.clearValidate()
}

const openEdit = (row) => {
  dialogTitle.value = '编辑年检'
  normalizeEditForm(row)
  dialogVisible.value = true
  if (formRef.value) formRef.value.clearValidate()
}

const submit = async () => {
  if (!formRef.value) return
  await formRef.value.validate(async (valid) => {
    if (!valid) return
    submitting.value = true
    try {
      await saveVehicleInspection({ ...form })
      ElMessage.success(form.id ? '年检更新成功' : '年检新增成功')
      dialogVisible.value = false
      await loadList()
    } finally {
      submitting.value = false
    }
  })
}

const handleDelete = (row) => {
  ElMessageBox.confirm(`确定删除年检记录【${row.certNo}】吗？`, '删除确认', {
    confirmButtonText: '确定删除',
    cancelButtonText: '取消',
    type: 'warning',
    center: true,
  })
    .then(async () => {
      await deleteVehicleInspection(row.id)
      ElMessage.success('年检记录已删除')
      await loadList()
    })
    .catch(() => {})
}

onMounted(async () => {
  await Promise.all([loadVehicleOptions(), loadDictOptions()])
  await loadList()
})
</script>

<style scoped>
.panel-root {
  height: 100%;
  display: flex;
  flex-direction: column;
  gap: 12px;
}

.header-actions {
  display: flex;
  justify-content: space-between;
  align-items: center;
  gap: 12px;
}

.search-group {
  display: flex;
  align-items: center;
  gap: 10px;
}

.search-input {
  width: 180px;
}

.search-select {
  width: 150px;
}

.mac-button-blue {
  background-color: #007aff !important;
  border-color: #007aff !important;
  border-radius: 8px !important;
  color: #fff;
}

.mac-button-gray {
  background-color: #e5e5ea;
  border-color: transparent;
  color: #1d1d1f;
  border-radius: 8px;
}

:deep(.el-table) {
  --el-table-border-color: transparent;
  --el-table-header-bg-color: transparent;
  --el-table-header-text-color: #86868b;
  flex: 1;
  overflow: hidden;
}

:deep(.el-table th.el-table__cell) {
  font-weight: 600;
  font-size: 13px;
}
</style>
